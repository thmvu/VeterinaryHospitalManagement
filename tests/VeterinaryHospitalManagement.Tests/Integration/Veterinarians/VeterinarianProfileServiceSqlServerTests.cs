...
public sealed class VeterinarianProfileServiceSqlServerTests
{
 [IdentitySqlServerFact] public async Task Create_normalizes_values_and_writes_audit(){await IdentitySqlServerTestEnvironment.RecreateAndMigrateAsync();using var f=new IdentitySqlServerWebApplicationFactory();var actor=await Start(f);var user=await Add(f,actor,"doctor1@vet.test",SystemRoleNames.Veterinarian);await With(f,s=>s.CreateAsync(new(actor,user," Nội khoa ")));await using var scope=f.Services.CreateAsyncScope();var db=scope.ServiceProvider.GetRequiredService<Web.Data.ApplicationDbContext>();var p=await db.VeterinarianProfiles.SingleAsync();Assert.Equal("VET-000001",p.DoctorCode);Assert.Equal("Nội khoa",p.Specialty);Assert.Single(await db.AuditLogs.Where(x=>x.Action=="VeterinarianProfile.Created").ToListAsync());}
 [IdentitySqlServerFact] public async Task Create_rejects_non_veterinarian_inactive_and_already_linked_users(){await IdentitySqlServerTestEnvironment.RecreateAndMigrateAsync();using var f=new IdentitySqlServerWebApplicationFactory();var actor=await Start(f);var receptionist=await Add(f,actor,"rec@vet.test",SystemRoleNames.Receptionist);await Assert.ThrowsAsync<VeterinarianManagementException>(()=>With(f,s=>s.CreateAsync(new(actor,receptionist,null))));var inactive=await Add(f,actor,"inactive@vet.test",SystemRoleNames.Veterinarian);await using(var scope=f.Services.CreateAsyncScope()){var db=scope.ServiceProvider.GetRequiredService<Web.Data.ApplicationDbContext>();(await db.Users.FindAsync(inactive))!.IsActive=false;await db.SaveChangesAsync();}await Assert.ThrowsAsync<VeterinarianManagementException>(()=>With(f,s=>s.CreateAsync(new(actor,inactive,null))));var active=await Add(f,actor,"active@vet.test",SystemRoleNames.Veterinarian);await With(f,s=>s.CreateAsync(new(actor,active,null)));await Assert.ThrowsAsync<VeterinarianManagementException>(()=>With(f,s=>s.CreateAsync(new(actor,active,null))));}
 [IdentitySqlServerFact] public async Task Sequential_creates_receive_distinct_generated_codes(){await IdentitySqlServerTestEnvironment.RecreateAndMigrateAsync();using var f=new IdentitySqlServerWebApplicationFactory();var actor=await Start(f);var a=await Add(f,actor,"a@vet.test",SystemRoleNames.Veterinarian);var b=await Add(f,actor,"b@vet.test",SystemRoleNames.Veterinarian);await With(f,s=>s.CreateAsync(new(actor,a,null)));await With(f,s=>s.CreateAsync(new(actor,b,null)));await using var scope=f.Services.CreateAsyncScope();var codes=await scope.ServiceProvider.GetRequiredService<Web.Data.ApplicationDbContext>().VeterinarianProfiles.OrderBy(x=>x.DoctorCode).Select(x=>x.DoctorCode).ToListAsync();Assert.Equal(["VET-000001","VET-000002"],codes);}
