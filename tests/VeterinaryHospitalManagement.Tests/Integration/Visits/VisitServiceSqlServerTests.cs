...
public sealed class VisitServiceSqlServerTests
{
    ...
    private static async Task<TestSetupData> Setup(IdentitySqlServerWebApplicationFactory factory)
    {
        ...
        var vetProfile1 = await db.VeterinarianProfiles.Where(x => x.UserId == vetUser1).Select(x => x.Id).SingleAsync();
        ...
        var vetProfile2 = await db.VeterinarianProfiles.Where(x => x.UserId == vetUser2).Select(x => x.Id).SingleAsync();
