...
public sealed class VeterinariansController(IVeterinarianProfileService service):Controller
{
 ...
 [HttpPost,ValidateAntiForgeryToken] public async Task<IActionResult> Edit(EditVeterinarianViewModel m,CancellationToken ct){if(!ModelState.IsValid)return View(m);try{await service.UpdateAsync(new(Current(),m.Id,Decode(m.RowVersion),m.Specialty),ct);TempData["StatusMessage"]="Đã cập nhật hồ sơ bác sĩ.";return RedirectToAction(nameof(Edit),new{id=m.Id});}catch(VeterinarianManagementException e){ModelState.AddModelError(string.Empty,e.Message);return View(m);}}
