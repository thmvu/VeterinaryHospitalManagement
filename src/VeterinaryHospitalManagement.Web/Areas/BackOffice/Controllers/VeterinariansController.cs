...
public sealed class VeterinariansController(IVeterinarianProfileService service):Controller
{
 ...
 [HttpGet,PermissionAuthorize(PermissionCodes.UserManage)] public IActionResult Create()=>View(new CreateVeterinarianViewModel());
 [HttpPost,ValidateAntiForgeryToken,PermissionAuthorize(PermissionCodes.UserManage)] public async Task<IActionResult> Create(CreateVeterinarianViewModel m,CancellationToken ct){if(!ModelState.IsValid)return View(m);try{await service.CreateWithAccountAsync(new(Current(),m.FullName,m.Email,m.Password,m.Specialty),ct);TempData["StatusMessage"]="Đã tạo tài khoản và hồ sơ bác sĩ.";return RedirectToAction(nameof(Index));}catch(VeterinarianManagementException e){ModelState.AddModelError(string.Empty,e.Message);return View(m);}}
